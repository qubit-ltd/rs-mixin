...
fn test_info_with_entity_clone_partial_eq() {
    let row = InfoWithEntity::new(None, "x".to_string(), "y".to_string(), None, Some("ORG".to_owned()));
    let row2 = row.clone();
    assert_eq!(row, row2);
}
...
fn test_info_with_entity_converts_from_and_into_info() {
    ...
    let row = InfoWithEntity::from_info(info.clone(), Some("CATEGORY".to_owned()));
...
fn test_info_with_entity_is_complete_requires_info_and_entity() {
    ...
    let missing_entity = InfoWithEntity::new(Some(1), "C".to_owned(), "N".to_owned(), Some(now), None);
    let missing_info = InfoWithEntity::new(None, "C".to_owned(), "N".to_owned(), Some(now), Some("ORG".to_owned()));
...
fn test_info_with_entity_sorts_by_info_then_entity() {
    let now = Utc::now();
    let mut rows = [
        InfoWithEntity::new(Some(2), "A".to_owned(), "A".to_owned(), Some(now), Some("A".to_owned())),
        InfoWithEntity::new(Some(1), "A".to_owned(), "A".to_owned(), Some(now), Some("B".to_owned())),
        InfoWithEntity::new(Some(1), "A".to_owned(), "A".to_owned(), Some(now), Some("A".to_owned())),
    ];
